...
#########################################################################################
	...
	Copyright: Fellowship Church
		All Rights Reserved.
	
	Software: Qube!

	Purpose: Elevate queuing algoritm replacement perl module.

        This provides a balance of clusters and priorities to efficiently
        utilize the render nodes in the farm.
...
sub qb_jobcmp
{
	...
	my $jobb = shift;
	my $host = shift;
	my $criticalThreshold = 10;

    # First check the priority level if it's in the critical threshold
    if ($jobb->{priority} < 
