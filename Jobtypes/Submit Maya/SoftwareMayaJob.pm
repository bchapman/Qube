...
sub initialize
{
	...
	if(not $self->SUPER::initialize()) {
		warn "ERROR: SUPER::initialize() returns false\n";
		warn "ERROR: cannot continue";
		return 0;
	}
...
sub processWork
{
	...
	if (defined($self->package->cameraOverride())) {
		warn "INFO: using camera specified in package\n";
		$cameraArray = ref($self->package->cameraOverride()) 
			? $self->package->cameraOverride() 
			: [$self->package->cameraOverride()];
	} elsif($work->package() and $work->package->cameraOverride()) {
		warn "INFO: using camera specified in work\n";
		$cameraArray = ref($work->package->cameraOverride()) 
			? $work->package->cameraOverride() 
			: [$work->package->cameraOverride()];
	} else {
		warn "INFO: using all renderable camera\n";
	...
	foreach my $camera (@$cameraArray) {
		...
		if($retval) {
			...
			if (!-e $result) {
				...
				$retval = 0;
			} elsif($self->package->badFrameSize() and
					(-s $result <= $self->package->badFrameSize())) {
				# check the filesize for at least "badFrameSize" bytes
				warn("ERROR: filesize [" . (-s $result) . " bytes] smaller ".
					 "than threshold [" .$self->package->badFrameSize().
					 " bytes]");
				$retval = 0;
			}
