...
sub processWork
{
	...
	$self->melexec("setAttr TurtleRenderGlobals.startFrame $frame");
	$self->melexec("setAttr TurtleRenderGlobals.endFrame $frame");
	$self->melexec("setAttr TurtleRenderGlobals.byFrameStep 1");
...
# 	}
	...
	$retval = $self->melexec('ilrBatchRender("")');
	my $result = $self->melresult();
...
sub setupRenderGlobals
{
	...
	# load turtle plugin if necessary
	$self->loadTurtlePlugin();

	$self->melexec("ilrDefaultNodes(1)");

	# load our custom mel script that overrides Turtle's original
	# ilrRenderBatchCallback6() proc
	my $qube_ilrRender_mel = qb::jobconfig("maya", "qube_ilrRender_mel", 1);
	$qube_ilrRender_mel =~ s/\\/\//g;
	$self->melexec("source \"$qube_ilrRender_mel\"");
...
sub loadTurtlePlugin
{
	my $self = shift;
	my $mayaver = $self->getMayaVersion();
	my $pluginName = "TurtleForMaya";
